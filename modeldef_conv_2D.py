...
#    n_length = np.sqrt(np.int(x.get_shape()[1]))
    n_length = 64
    ...
    with tf.variable_scope('conv_1'):
        # Convolution Layer
        conv1 = tf.nn.tanh(batch_norm_layer(conv2d(x_cplx, weights['wc1'], biases['bc1']), train_phase=train_phase, scope_bn='bn1'))
        # Max Pooling (down-sampling)
        conv1 = maxpool2d(conv1, k=2)

    with tf.variable_scope('conv_2'):        
        # Convolution Layer
        conv2 = tf.nn.tanh(batch_norm_layer(conv2d(conv1, weights['wc2'], biases['bc2']), train_phase=train_phase, scope_bn='bn2'))
...
def get_accuracy(pred, y):
    with tf.name_scope('accuracy-node'):
        ...
        tf.scalar_summary("accuracy_weak", accuracy_weak)
