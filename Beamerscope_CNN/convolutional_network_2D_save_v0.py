...
import modeldef_conv_2D_Training as mod
import input_data as indata
import time


# num of iterations
num_steps = 10000  # 272640
batch_size = 128
...
train_x_cplx_2D, train_y, test_x_cplx_2D, test_y, valid_x_cplx_2D, valid_y = indata.handle_input_data(read_data, preprocess_data, inputmat_name, outputmat_name)
...
with tf.device('/gpu:0'):
    ...
    #        print("Model saved in file: %s" % save_path)                      
        ...
        if step % display_step == 0:
            ...
            offset_valid = offset
            batch_valid_x_cplx_2D = valid_x_cplx_2D[offset_valid:(offset_valid + batch_size), :, :, :] # np.random.rand(128, n_input) # 
            batch_valid_y = valid_y[offset_valid:(offset_valid + batch_size), :] # np.random.rand(128, 48) #
        
            loss, weak_acc, acc, summary = sess.run([cost, correct_pred, accuracy, summary_op], feed_dict={x_cplx: batch_valid_x_cplx_2D, y: batch_valid_y, keep_prob: 1.,  train_phase: False})
    ...
    # final iteration
    offset=0        
    batch_test_x_cplx_2D = test_x_cplx_2D[offset:(offset + batch_size), :, :, :] # np.random.rand(128, n_input) # 
    batch_test_y = test_y[offset:(offset + batch_size), :] # np.random.rand(128, 48) #
    loss, weak_acc, acc, summary = sess.run([cost, correct_pred, accuracy, summary_op], feed_dict={x_cplx: batch_test_x_cplx_2D, y: batch_test_y, keep_prob: 1.,  train_phase: False})
    print("Final Result: ")
