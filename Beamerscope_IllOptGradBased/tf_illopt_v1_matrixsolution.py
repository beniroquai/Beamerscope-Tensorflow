...
with tf.device('/gpu:0'):
    ...
    tf_I = tf_I + tf.reduce_sum(tf_eigenval_i*tf.abs(tf.conj(tf_aerial_FT)*tf_aerial_FT), [0, 1])
    ...
    for object_iter in range(0,n_samples):
...
tf_aerial_FT = (tf.ifft2d(tf_ifftshift(tf_ft_Test)))
i_object, ft_Test, aerial_FT, aerial = sess.run([tf_object, tf_ft_Test, tf_aerial_FT, tf_aerial], feed_dict = {tf_object_real: object_i_real, tf_object_imag: object_i_imag})
        
object_FT = sess.run(tf_object_FT, feed_dict = {tf_object_real: object_i_real, tf_object_imag: object_i_imag})
