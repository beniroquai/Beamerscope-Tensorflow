...
inputmat_name = './nninputs_realimag.mat'
outputmat_name = './nnoutputs.mat'
...
weights = {
    ...
    'out': tf.Variable(tf.random_normal([n_fully, n_classes]), name = 'out')
}
...
writer = tf.train.SummaryWriter('./newgraph', graph=tf.get_default_graph())
...
with g_2.as_default():
    ...
    tf.train.write_graph(graph_def, logs_path, 'expert-graph.pb', as_text=False)
